...
const index: FC<Props> = ({ timer, INITTIMER }) => {
	return (
		<div className='w-100' style={{ height: '1.5rem' }}>
			<div className='rounded w-100 bg-white' style={{ height: '100%' }}>
				<div
					className={`rounded ${
						timer > INITTIMER - INITTIMER / 4 && 'bg-good'
					} ${
						timer <= INITTIMER - INITTIMER / 4 &&
						timer > INITTIMER / 2 - INITTIMER / 4 &&
						'bg-middle'
					} ${timer <= INITTIMER / 4 && 'bg-bad'}`}
					role='progressbar'
					style={{
