...
const Game = () => {
	...
	const pickQuestion = useCallback(async () => {
		...
		const {
			results: {
				0: { category, correct_answer, incorrect_answers, question },
			},
		} = res;
	...
	return (
		<>
			...
			<div className='category-question text-center text-white fs-3 pt-5'>
				<p className='m-0'>{question?.category}</p>
			</div>
			<div className='container'>
				...
				<div className='container-question'>
					...
					<div className='third d-flex justify-content-center align-items-center text-center px-2'>
						{question && (
							<p
								className='text-black fs-5 px-2'
				...
				<div className='container-answer w-100 mt-3'>
					{question?.answers.map((answer: string, index: number) => (
						<button
							...
							className={`text-black rounded w-100 fs-5 ${
								userAnswer === 'Incorrect' &&
